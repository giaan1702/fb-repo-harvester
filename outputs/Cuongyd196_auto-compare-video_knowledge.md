# 📦 Tri Thức Kỹ Thuật Dự Án: Cuongyd196/auto-compare-video

> **Mô tả ngắn**: 🎬 Tạo video "so sánh kiến thức" ngắn tự động — HyperFrames + AI voice, 1 template nhiều chủ đề.

## 1. Nguồn Giới Thiệu & Video
- **Link bài viết / Video gốc**: [https://www.facebook.com/reel/1855108012137489](https://www.facebook.com/reel/1855108012137489)
- **Kênh / Người chia sẻ**: Cộng đồng Công Nghệ

## 2. Thông Tin Kho Mã Nguồn (GitHub Metadata)
- **GitHub URL**: [https://github.com/Cuongyd196/auto-compare-video](https://github.com/Cuongyd196/auto-compare-video)
- **Chỉ số cộng đồng**: ⭐ **165** stars | 🍴 **94** forks
- **Ngôn ngữ chủ đạo**: `HTML`
- **Giấy phép bản quyền (License)**: `MIT`
- **Chủ đề (Topics)**: `aiagent`, `claude-code`, `claude-skills`, `codetovideo`, `cuongit`, `hyperframes`, `videoai`
- **Cập nhật gần nhất**: 2026-09-04T03:41:31Z

## 💡 3. Nhận Định & Phân Loại Của AI
- **Phân Loại Lĩnh Vực**: `Computer Vision & Generative Media`
- **Kiến Trúc Kỹ Thuật**: `Template-based Video Generation Pipeline`
...
- **Các Thành Phần / API Cốt Lõi**:
  * HyperFrames template file
  * AI voice synthesis script (using Claude/Azure TTS)
  * JSON configuration for topic pairs

## 🚀 4. Hướng Dẫn Cài Đặt Nhanh (Technical Quickstart)
```bash
git clone https://github.com/Cuongyd196/auto-compare-video.git && cd auto-compare-video && npm install
```

## 💻 5. Mã Nguồn Mẫu Thực Chiến ("Hello World")
```
const { renderVideo } = require('@hyperframes/cli');

async function generateComparisonVideo(topicA, topicB) {
  const config = {
    template: 'comparison-template.hf',
    data: { left: topicA, right: topicB },
    voice: { provider: 'azure', language: 'vi-VN', name: 'vi-VN-HoaiNeural' }
  };
  await renderVideo(config, 'output/video.mp4');
  console.log('Video rendered:', 'output/video.mp4');
}

// Example usage
generateComparisonVideo('Thiên thạch', 'Sao băng');
```
