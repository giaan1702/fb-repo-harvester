# 📦 Tri Thức Kỹ Thuật Dự Án: ultraworkers/claw-code

> **Mô tả ngắn**: An agent-managed museum exhibit, built in Rust with Gajae-Code / LazyCodex — developed and maintained with no human intervention.

## 1. Nguồn Giới Thiệu & Video
- **Link bài viết / Video gốc**: [https://www.facebook.com/reel/2172650416653323](https://www.facebook.com/reel/2172650416653323)
- **Kênh / Người chia sẻ**: Cộng đồng Công Nghệ

## 2. Thông Tin Kho Mã Nguồn (GitHub Metadata)
- **GitHub URL**: [https://github.com/ultraworkers/claw-code](https://github.com/ultraworkers/claw-code)
- **Chỉ số cộng đồng**: ⭐ **195,170** stars | 🍴 **108,699** forks
- **Ngôn ngữ chủ đạo**: `Rust`
- **Giấy phép bản quyền (License)**: `MIT`
- **Cập nhật gần nhất**: 2026-09-05T14:33:31Z
...
```bash
git clone https://github.com/ultraworkers/claw-code.git && cd claw-code && cargo build --release
```

## 💻 5. Mã Nguồn Mẫu Thực Chiến ("Hello World")
```rust
use claw_code::{Agent, Exhibit};

#[tokio::main]
async fn main() {
    // Khởi tạo agent với tên "claw-bot"
    let mut agent = Agent::new("claw-bot");
    
    // Tải mô tả triển lãm từ cấu hình
    let exhibit = Exhibit::load("museum_hall");
    
    // Gắn agent vào triển lãm
    agent.attach_exhibit(exhibit).await;
    
    // Bắt đầu vòng lặp tự trị của agent
    agent.run().await;
}
```
