# 📦 Tri Thức Kỹ Thuật Dự Án: FoundationAgents/OpenManus

> **Mô tả ngắn**: No fortress, purely open ground.  OpenManus is Coming.

## 1. Nguồn Giới Thiệu & Video
- **Link bài viết / Video gốc**: *Được thu thập từ mạng xã hội/cộng đồng*
...
- **Chỉ số cộng đồng**: ⭐ **58,195** stars | 🍴 **10,102** forks
- **Ngôn ngữ chủ đạo**: `Python`
- **Giấy phép bản quyền (License)**: `MIT`
- **Cập nhật gần nhất**: 2026-09-05T14:48:26Z
...
- **Các Thành Phần / API Cốt Lõi**:
  * Agent (luồng điều phối và quyết định)
  * Planner (sinh kế hoạch hành động từ mục tiêu)
  * ToolExecutor (thực hiện các công cụ như truy vấn web, chạy code)
  * Memory (bộ nhớ ngắn hạn và dài hạn để lưu trữ bối cảnh)
...
```python
from openmanus import Agent, Planner, ToolExecutor, Memory\n\n# Khởi tạo thành phần\nmemory = Memory()\nplanner = Planner()\ntool_executor = ToolExecutor()\nagent = Agent(planner, tool_executor, memory)\n\n# Định nghĩa mục tiêu\ngoal = "Tìm kiếm thông tin về missão Artemis của NASA và tóm tắt trong 3 câu."
